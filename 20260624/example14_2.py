...
def run_data_quality_audit(orders, order_items, sessions, events, customers):
    ...
    try:
        ...
        invalid = valid_event_types-actual_types 
        
        # 若有無效的資料顯示警告文字 否則 顯示 全數通過
        if invalid :
            print(f'發現非法event_type:\n{invalid}')
        else :
            print('一切合法')
    except Exception as e:
         print(f" 檢查失敗: {e}")
    ...
    try:
        ...
        merged = orders.merge(events[events['revenue']>0],on='order_id',how='inner')
        ...
        if count > 0:
            ...
            print(merged[['order_id','status','revenue']])
    except Exception as e:
         print(f"   檢查失敗: {e}")
